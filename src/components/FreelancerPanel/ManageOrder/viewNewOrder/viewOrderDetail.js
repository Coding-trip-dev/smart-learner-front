...
import { getNewOrders } from '../../../../redux/actions/freelancer/orderAction'
...
const ViewRunningOrder = props => {
  const { getNewOrders, newOrders } = props
  ...
  React.useEffect(() => {
    ...
    if (newOrders) {
      ...
      let selectedOrder = newOrders.filter((item, key) => {
            ...
            <div className="user-detail-right">
              <ul className="detail-heading">
                ...
                <li className="li">
                  {
                    order ?
                      order.source_files.length > 0 ?
                        order.source_files.map((item, key) => {
...
export default withRouter(connect(mapStateToProps, { getNewOrders })(ViewRunningOrder))
