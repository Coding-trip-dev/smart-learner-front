...
import { getAllChapter, updateTopic } from '../../../../redux/actions/instructor/chapterDetailsActionInstructor'
import {
    getAllCourse, 
    addNewChapter,
    getAllTopicsOfQuestions
}
    from '../../../../redux/actions/instructor/courseDetailsActionInstructor'
...
const ViewCourseDetails = props => {
    ...
    useEffect(() => { 
        ...
        getCourses({ instructor_id: auth.user_id },'instructor')

        getAllQuestions({ course_id: props.match.params.id },'instructor')

        setTimeout(() => {
            if (!chapterRedu.allChapter) {
                getChapters(newObj, 'instructor')
            }
