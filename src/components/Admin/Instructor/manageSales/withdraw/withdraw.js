...
import { getWirhdrawlHistory, getBankDetail, sendWithdrawRequest } from '../../../../../redux/actions/instructor/salesAction'
...
const Withdraw = props => {
  ...
  React.useEffect(() => {
    getWirhdrawlHistory({ instructor_id: "5ea928f88421aa125c81c179" })
    getBankDetail({ instructor_id: "5ea928f88421aa125c81c179" })
  }, [])
  React.useEffect(() => {
  ...
  const addBankAcc = e => {
    fetch(baseURL + "/api/instructor/withdraw/get-oauth-link", {
      method: "GET",
      headers: {
      ...
      .then(data => {
        if (data.url) {
          window.location = data.url + "&scope=read_write&redirect_uri=http://127.0.0.1:3000/instructor/sales/withdraw";
  ...
  useEffect(() => {
    ...
    if (code) {
      ...
      let body = {
        userId: "5ea928f88421aa125c81c179" // temp id
        // id 
      }
      axios.post(baseURL + "/api/instructor/withdraw/authorize-oauth" + code, body)
        .then(response => {
          console.log(response.data.data)
          if (response.data.data.resp) {
            toast.success("Your bank account has been successfully added.")
            setTimeout(() => {
              window.location = 'http://127.0.0.1:3000/instructor/sales/withdraw'
  ...
  const sendRequest = e => {
    let newState = {
      instructor_id: "5e9d4a5eab438002fc7d97df",
